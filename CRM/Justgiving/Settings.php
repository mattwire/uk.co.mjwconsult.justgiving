...
class CRM_Justgiving_Settings {

  CONST TITLE = 'JustGiving';
   ...
   * @param array $values Array of settings and values with or without prefix (eg. array(justgiving_username => 'test')) to save
   */
  ...
  public static function getValue($name) {
    $settings = civicrm_api3('setting', 'get', array('return' => CRM_Justgiving_Settings::getName($name,true)));
    $domainID = CRM_Core_Config::domainID();
    if (isset($settings['values'][$domainID][CRM_Justgiving_Settings::getName($name,true)])) {
      return $settings['values'][$domainID][CRM_Justgiving_Settings::getName($name, true)];
    }
    return '';
  }
